...
double f(double x){
        return 1569887*exp(x)+(280246/x)*(exp(x)-1)-3581685;
...
void newton(double (*f)(double), double (*df)(double),double x0, int n){
    ...
    double df(double x){
        return 1569887 * exp(x) + 280246 * (exp(x) * x - exp(x) + 1)/(x*x); //calular a derivada em algum outro software, VAI NO WOLFAMALPHA, ESCREVE DERIVATIVE NA FRENTE DA FUNÇAO, CLICA EM CALCULAR, DEPOIS POROCURA A ABA "ALTERNATE FORMS", CLICA EM PLAIN TEXT, CLICA EM CIMA DA FUNÇAO QUE ESTA NO CAMPO "COPYABLE PLAIN TEXT"
    };
...
int main() {
    //bissecao
    printf("\n/////////////////////////////bissecao/////////////////////////////\n");
    double a =  0.1;  // intervalo dado ou chutado para fazer o teorema de bolzano               
    double b =   1.43;// e descobrir se aqui tem ou nao uma raiz
    ...
    //secante
    printf("\n/////////////////////////////secante/////////////////////////////\n");
   double x_ini = 0.1; //primeiro ponto chutado
   double x_dois = 1.92; //segundo ponto chutado
   int o = 5; //numero de iteraçoes

   secante(f, x_ini,x_dois, o);


    //posfalsa
    printf("\n/////////////////////////////posicao falsa/////////////////////////////\n");
    double comeco_intervalo = 0.1;  // primeiro ponto chutado
    double final_intervalo = 1.57;  // segundo ponto chutado
