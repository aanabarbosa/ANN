...
void bissection(double (*f)(double), double a, double b, double n,
                double tol) {  //
    ...
    if (fa * fb >= 0) {  // verificaçao dp teorema em si
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (fa * fm <= 0) {  // Verifica se f(m) está do lado aposto de fa
                                 // em realcao ao eixo x.
...
double f(double x) {
    return 1-((pow(191.72,2))/(9.81*pow(2.33*x+(x*x)/2,3)))*(2.33+x);
...
int main() {
    ...
    printf(
        ...
        "\n");
    double a =
        0.78;  // intervalo dado ou chutado para fazer o teorema de bolzano
    double b = 9.13;  // e descobrir se aqui tem ou nao uma raiz
    ...
    printf(
        "\n/////////////////////////////posicao "
        "falsa/////////////////////////////\n");
    double comeco_intervalo = 0.79;  // primeiro ponto chutado
    double final_intervalo = 9.6;   // segundo ponto chutado
